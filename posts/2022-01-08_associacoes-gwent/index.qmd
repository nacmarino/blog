...
decks %>% 
  ...
  geom_bar(position = 'fill', stat = 'identity', color = 'black', width = 0.9, show.legend = FALSE) +
...
```{r figura_cartas_comuns_neutras}
#| column: page
#| fig-width: 10
#| fig-height: 4
#| code-fold: true
...
df_regras %>% 
  ...
  mutate(across(.cols = c(support, confidence, coverage, lift, x, y), \(x) round(x, digits = 3))) %>% 
