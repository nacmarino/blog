...
::: {.callout-note}
Este *post* foi o meu trabalho de conclusão do curso **Relatórios e visualização de dados** que fiz em julho de 2021 na Curso-R [Curso-R](https://loja.curso-r.com/relatorios-e-apresentacoes-automaticas.html), que foi escolhido como um dos três melhores trabalhos da [turma](https://curso-r.github.io/202107-visualizacao/).
:::
...
dados %>% 
  ...
  ggplot(mapping = aes(values = prop, fill = Bairro)) +
  geom_waffle(n_rows = 15, make_proportional = FALSE, flip = TRUE, 
              color = 'white', size = 1.125) +
