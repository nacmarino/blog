...
tempos_de_prova %>% 
  ...
  geom_smooth(formula = y ~ x, se = FALSE, color = 'firebrick3', method = 'loess') +
...
velocidades_por_prova <- resultados %>%
  ...
  mutate(fastestLapSpeed = parse_number(fastestLapSpeed, na = '\\N')) %>% 
  # dropando NA na variavel alvo
...
df_regs %>% 
  ...
  geom_smooth(formula = y ~ x, color = 'black', linetype = 1, method = 'lm', se = TRUE) +
  labs(
